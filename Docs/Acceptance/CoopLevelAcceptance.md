...
本次联机没有创建独立竞技场，而是在原有 `Level_01` 单机玩法上增加三种启动方式：

- 单机游戏：使用 NGO 原生 `SinglePlayerTransport`，不监听 UDP 端口，保留存档和全局暂停。
- 创建 Host：本机同时作为服务器与一号玩家，监听 UDP `7777`。
- 加入 Client：输入 Host 的局域网 IPv4；同一台电脑双开时填写 `127.0.0.1`。
...
2. 左侧窗口点击“创建双人 Host”。
3. 等待左侧生成蓝色玩家，右上角显示 `Host 运行中（1/2）`。
4. 右侧窗口在 `Host IPv4` 输入框填写 `127.0.0.1`。
5. 点击“加入双人 Client”。
6. 等待右侧生成橙色本机玩家；两个窗口都应看到两名玩家。
7. Host 面板应显示 `2/2`，Client 面板应显示已连接和 RTT。
...
1. 两名玩家一起接近第一战区。
2. 确认 Chomper 会选择较近的存活玩家追击并近战攻击。
...
7. 观察右上角调试面板，确认显示 RTT、权威生命、攻击序号以及 Host 接受或拒绝原因。
...
6. 关闭 Host，Client 应退出会话并显示中文断开原因。
...
3. Esc 或 P 可以暂停整个单机游戏，`Time.timeScale` 变为零。
4. 保存后改变位置与生命，再读取存档，玩家快照应正确恢复。
5. 联机模式下打开本地菜单不得暂停另一个窗口，保存和读取操作必须被中文提示拒绝。
