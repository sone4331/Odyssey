...
3. 进入 Play Mode 后观察 Inspector 的只读区域：当前行为、中文行为树路径、感知状态、目标距离和生命比例会持续刷新。
4. 怪物始终与战区激活状态无关：未感知玩家时沿六点宽范围网络巡逻，进入黄色发现范围后追击，进入红色攻击范围且冷却完成后攻击。
5. 玩家离开橙色丢失范围后，怪物应放弃目标并恢复巡逻，边界附近不应逐帧抖动切换。
6. Spitter 进入青色最小安全距离时后撤；Chomper 不因低生命自动撤退。
7. 受击与死亡分支通过响应式选择器抢占巡逻、追击或攻击，并清理旧导航路径和攻击前摇。
