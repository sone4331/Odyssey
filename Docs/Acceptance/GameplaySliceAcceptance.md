...
- 接管第一战区附近的原 PressurePad 与 DoorHuge，配置项目自有清怪门禁。
...
场景中不应再出现项目生成的蓝色战斗出口或玩家进入触发区。原关卡 PressurePad 上应有 `EncounterClearancePressurePlate`，原 DoorHuge 上应有 `EncounterClearanceGate`；第三方发送命令与门体平移组件在选中对象内保持禁用，避免双重控制。
...
- 达标后冻结单机功能，下一阶段只在独立 `NetworkArena` 场景实现 Host 权威状态同步。
