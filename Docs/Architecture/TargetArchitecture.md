...
| 会话 | 从开始一局游戏或建立一次连接，到返回菜单或断开连接 | ActorRegistry、权威战斗上下文；出现真实状态前不创建空壳类 |
...
命令表达“请求执行”，事件表达“已经发生”。命令使用一对一方法调用并返回结果；事件优先使用拥有者公开的强类型 `Action<T>` 通知直接观察者。

当前规模不保留通用 EventBus。只有当同一领域事实出现三个以上彼此独立的生产消费者，并且需要统一事件历史或调试监视时，才重新评估会话级类型化总线。禁止为了未来可能的 UI、音频或网络需求预建无消费者基础设施。
