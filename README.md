...
- 输入适配器只暴露连续快照和离散动作命令；光标、暂停、本机玩家输入与 FreeLook 输入统一由一个场景菜单控制器提交。
- `Level_01` 使用全屏中文主菜单进入单人或联机模式；单机使用无端口 `SinglePlayerTransport`，联机使用 Unity Transport（UDP `7777`）。
- 双人合作复用原玩家手感、两组 Chomper/Spitter、行为树、投射物、战区和清怪门禁；Owner 负责移动表现，Host 权威决定攻击、生命、AI、投射物、复活和关卡状态。
- 网络攻击使用递增请求序号，并明确拒绝重复、过期、非法连击、冷却中、无目标和超距离请求；按 F3 可显示或隐藏 RTT、连接人数和权威生命调试信息。
- 当前门禁为 55 项纯 C# 核心规格、31 项 Unity EditMode 测试和 24 项 PlayMode 运行态测试。

正常游玩入口为：`单人游戏` 直接开局；`联机游戏 → 创建房间/加入房间` 建立双人会话；游戏内按 ESC 打开继续、设置、单机存读档或离开联机菜单。单机菜单暂停世界，联机菜单只关闭本机玩家与镜头输入，不会暂停另一名玩家和 Host 权威模拟。

首个作品集版本明确不实现 Lua、完整 GAS、确定性帧同步、KCP、Relay、NAT 穿透、通用对象池或全量零 GC 框架。联机范围冻结在可本机双开和局域网直连的双人技术切片，不继续扩展为在线服务框架。

## 架构文档

- [中文技术学习指南](Docs/Guides/README.md)：面向 Unity 开发者的源码阅读入口，按启动、玩法、AI、数据、联机、测试和面试准备组织。
...
- [原关卡双人合作联机验收指南](Docs/Acceptance/CoopLevelAcceptance.md)：从自动搭建、Windows 构建到本机双开逐项验证 Host 权威状态同步。
