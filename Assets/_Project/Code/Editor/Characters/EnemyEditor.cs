...
{
    /// <summary>
    /// 在 Inspector 与 Scene 视图展示怪物黑板、行为树运行路径和感知半径。
    ...
    public sealed class EnemyEditor : UnityEditor.Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            {
                EditorGUILayout.TextField("当前行为", enemy.CurrentGoal.ToString());
                EditorGUILayout.TextField("行为树路径", enemy.CurrentBehaviorPath);
                EditorGUILayout.Toggle("已感知玩家", enemy.HasSensedTarget);
        ...
        private void OnSceneGUI()
        {
            ...
            Handles.color = new Color(1f, 0.55f, 0f, 0.45f);
            Handles.DrawWireDisc(enemy.transform.position, Vector3.up, enemy.ForgetRange);
